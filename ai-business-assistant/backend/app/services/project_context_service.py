...
from app.db.models import AssetAnalysisResult, EmployeeMessage, EmployeeMessageAttachment, Project, ProjectMessage
...
def build_project_context(db: Session, project_id: int) -> dict:
    ...
    attachments = db.query(EmployeeMessageAttachment).filter(EmployeeMessageAttachment.project_id == project_id).order_by(EmployeeMessageAttachment.created_at).all()
    ...
    if attachments: sections.append("## 聊天附件\n" + "\n".join(f"- {a.original_filename}（{a.file_type}，{a.file_size} 字节）" for a in attachments))
    return {"project": project, "project_messages": project_messages, "asset_analysis_results": asset_results, "related_employee_messages": employee_messages, "message_attachments": attachments, "context_markdown": "\n\n".join(sections)}
