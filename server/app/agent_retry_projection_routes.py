...
def install_agent_retry_projection_routes() -> None:
    ...
    The normal ``GET /api/agent/tasks`` route already uses AgentTaskStore.list(), which Phase 7.39
    makes logical-task-only by default. This endpoint provides the durable internal-attempt audit
    view without changing old Android/API list response shapes.
    """
    ...
    async def get_retry_chain(task_id: str, request: Request) -> dict[str, object]:
        ...
        return {
            "task_id": task_id,
            "logical_task_id": str((chain or {}).get("root_task_id") or task_id),
