...
def _tool_definitions() -> dict[str, dict[str, Any]]:
    return {
        "vercel_list_projects": {
            ...
            "inputSchema": {"type": "object", "properties": {"search": {"type": "string"}, "limit": {"type": "integer", "minimum": 1, "maximum": 100}, "cursor": {"type": "string"}}, "additionalProperties": False},
        },
        "vercel_list_deployments": {
            ...
            "inputSchema": {"type": "object", "properties": {"project_id": {"type": "string"}, "limit": {"type": "integer", "minimum": 1, "maximum": 100}, "state": {"type": "string"}, "target": {"type": "string"}, "branch": {"type": "string"}}, "additionalProperties": False},
        },
        "vercel_read_deployment": {
            ...
            "inputSchema": {"type": "object", "properties": {"deployment_id": {"type": "string"}}, "required": ["deployment_id"], "additionalProperties": False},
        },
        "vercel_redeploy_preview": {
            ...
            "inputSchema": {"type": "object", "properties": {"project_id": {"type": "string"}, "deployment_id": {"type": "string"}}, "required": ["project_id", "deployment_id"], "additionalProperties": False},
        },
...
def install_vercel_mcp_tools() -> None:
    ...
    def authorized(owner_id: str, employee: dict[str, Any]) -> list[str]:
        ...
        # special-cased the literal "write" risk. Use the gateway binding so existing runtime/test
        # overrides keep one authorization truth.
        ...
        for name in names:
            ...
            mode = str(plugin_mcp_gateway.effective_agent_grant(owner_id, employee, plugin_id).get("mode") or "none") if plugin_id else "none"
    ...
    def catalog(owner_id: str, employee: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for row in rows:
            ...
            if str(spec.get("risk") or "") == "dangerous":
                ...
                row["annotations"] = {**annotations, "readOnlyHint": False, "destructiveHint": True, "idempotentHint": False}
        return rows

    @wraps(original_run_plugin_tool)
    def mcp_run_plugin_tool(owner_id: str, employee: dict[str, Any], plugin_id: str, tool_name: str, executor: Callable[[], Any], **kwargs: Any) -> Any:
        ...
        dangerous = any(
            ...
            for spec in plugin_mcp_gateway._TOOL_DEFINITIONS.values()
        )
        return original_run_plugin_tool(owner_id, employee, plugin_id, tool_name, executor, confirmed=bool(dangerous), **kwargs)
    ...
    def vercel_executor(owner_id: str, tool_name: str, arguments: dict[str, Any]) -> Callable[[], Any]:
        ...
        if tool_name == "vercel_list_deployments":
            ...
            return lambda: plugin_vercel.list_vercel_deployments(owner_id, project_id=project_id, limit=limit, state=state, target=target, branch=branch)
    ...
    def vercel_safe_result(value: Any) -> Any:
        if isinstance(value, dict) and value.get("plugin_id") == "vercel":
            allowed = {"plugin_id", "projects", "deployments", "deployment", "count", "pagination", "action", "project_id", "deployment_id"}
